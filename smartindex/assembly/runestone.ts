...
export class RunestoneParser {
  ...
  getHeightStart(): Option<u64> {
    return this.getFieldValue<u64>(Field.HEIGHTSTART);
  }

  getHeightEnd(): Option<u64> {
    return this.getFieldValue<u64>(Field.HEIGHTEND);
  }

  getOffsetStart(): Option<u64> {
    return this.getFieldValue<u64>(Field.OFFSETSTART);
  }

  getOffsetEnd(): Option<u64> {
    return this.getFieldValue<u64>(Field.OFFSETEND);
  }
    ...
    if (this.isEtching()) {
      ...
      if (this.isTerms()) {
        ...
        if (heightStart.isSome) {
          encoder.setInteger("height_start", heightStart.some);
        }
        const heightEnd = this.getHeightEnd();
        if (heightEnd.isSome) {
          encoder.setInteger("height_end", heightEnd.some);
        }
        const offsetStart = this.getOffsetStart();
        if (offsetStart.isSome) {
          encoder.setInteger("offset_start", offsetStart.some);
        }
        const offsetEnd = this.getOffsetEnd();
        if (offsetEnd.isSome) {
          encoder.setInteger("offset_end", offsetEnd.some);
        }
